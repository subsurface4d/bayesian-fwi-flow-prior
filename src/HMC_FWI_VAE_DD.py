...
from seisfwi.survey import Survey, Source, Receiver, SeismicData
...
from seisfwi.utils import wavelet
...
def run_workflow(args):
    ...
    with torch.no_grad():
        dsyn_int = F(model_fwi)

    # Double difference data
    dobs_dd = SeismicData(survey)
    dobs_dd.record_data({"pr": dsyn_int.data['pr'] + dobs_ml.data['pr'] - dobs_bl.data['pr']})

    fwi = DeterministicFWI(F, model_fwi, loss_fn, dobs_dd, weight)
    fwi.run(
        optimizer=optimizer,
        max_iter=max_iter,
    ...
    fwi = DeterministicFWI(F, model_fwi_sat, loss_fn, dobs_dd, weight)
    ...
    # # Initial model
    # model_fwi_hmc = AcousticModelVAE(ox, oz, dx, dz, nx, nz, 
    #                                 model_vae, 
    #                                 rock_physics_params,
    #                                 vp_bl, 
    #                                 latent_grad = True, 
    ...
    # # Setup priors
    # ndim = model_fwi_hmc.ndim

    # # Norm dist prior for the latent space
    # prior_mean = np.zeros(ndim)
    # prior_std  = np.ones(ndim)
    # prior = GaussianPrior(prior_mean, prior_std, ndim)

    # # Set temperature for HMC
    # # temp=0.075
    # # temp=0.050
    # # temp=0.025
    # # temp=0.010
    ...
    # # Set inference model
    # fwi_bayes = ProbabilisticFWI(F, model_fwi_hmc, loss_fn, dobs_dd, weight, prior, temp=temp)
