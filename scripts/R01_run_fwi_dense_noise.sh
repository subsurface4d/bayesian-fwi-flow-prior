...
for src_id in {1..1}; do
    for rec_id in {1..1}; do
        for noise_snr in 25 30; do
            # Construct output directory based on whether noise is used
            if [ "$noise_snr" == "None" ]; then
                out_dir="/net/vision/scr2/haipeng/FWI-HMC/Runs-Paper/SYN-survey-${src_id}-geophone-${rec_id}-${freq}Hz"
                noise_flag=""
            else
                out_dir="/net/vision/scr2/haipeng/FWI-HMC/Runs-Paper/SYN-survey-${src_id}-geophone-${rec_id}-${freq}Hz-noise-${noise_snr}dB"
