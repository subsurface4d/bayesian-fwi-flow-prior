...
for noise_snr in None 25 30; do
    for src_id in {2..4}; do
        for rec_id in {1..2}; do
            for freq in 30 60; do
                    for temp in 0.025; do
                    # Construct output directory based on whether noise is used
                    if [ "$noise_snr" == "None" ]; then
                        out_dir="/net/vision/scr2/haipeng/FWI-HMC-Revision/Runs-Paper/SYN-survey-${src_id}-geophone-${rec_id}-${freq}Hz"
                        noise_flag=""
                    else
                        out_dir="/net/vision/scr2/haipeng/FWI-HMC-Revision/Runs-Paper/SYN-survey-${src_id}-geophone-${rec_id}-${freq}Hz-noise-${noise_snr}dB"
                    ...
                    # Run the job
                    python src/HMC_FWI_VAE_DD.py --output_dir "$out_dir"  \
