        ...
        for rec_id in {1..2}; do
            for freq in 30; do
                    ...
                    if [ "$noise_snr" == "None" ]; then
                        out_dir="/net/vision/scr2/haipeng/FWI-HMC-Revision/Runs-Paper/SYN-VAE-Ones-survey-${src_id}-geophone-${rec_id}-${freq}Hz"
                        noise_flag=""
                    else
                        out_dir="/net/vision/scr2/haipeng/FWI-HMC-Revision/Runs-Paper/SYN-VAE-Ones-survey-${src_id}-geophone-${rec_id}-${freq}Hz-noise-${noise_snr}dB"
                    ...
                    # Run the job
                    python src/FWI_VAE.py --output_dir "$out_dir"  \
