...
def _get_hicks_locations_and_weights(
    ...
    # to avoid the zero location for empty source wavelet
    hicks_locations = torch.zeros(n_shots,
                                  n_per_shot_hicks,
                                  2,
                                  dtype=torch.long,
...
class Hicks:
    ...
    # def receiver(self, amplitudes: Tensor, shot_idxs: Optional[Tensor] = None) -> Tensor:
    #     _check_shot_idxs(amplitudes, shot_idxs)
    #     n_shots, _, nt = amplitudes.shape
    #     n_per_shot = self.locations.shape[1]

    #     if shot_idxs is None:
    #         hicks_shot_idxs = torch.arange(n_shots, device=amplitudes.device)
    #     else:
    #         hicks_shot_idxs = shot_idxs.long()

    #     selected_amplitudes = amplitudes[hicks_shot_idxs, self.idxs[hicks_shot_idxs].reshape(-1), :]
    #     weights_0 = self.weights[hicks_shot_idxs].reshape(n_shots, -1, 1)
    #     weights_1 = self.weights[hicks_shot_idxs].reshape(n_shots, -1, 1)

    #     result = selected_amplitudes * (weights_0 * weights_1).sum(dim=1)
    #     out = result.view(n_shots, n_per_shot, nt).sum(dim=1)
