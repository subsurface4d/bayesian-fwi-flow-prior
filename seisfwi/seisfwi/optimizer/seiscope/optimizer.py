...
class Optimizer(object):
    ...
    def __init__(self, niter_max = 20, conv = 1e-8, method = 'LBFGS', 
                 ...
                 lbfgs_memory = 5, nls_max=6, debug = False, log_file=None):
        '''
            ...
            :param str log_file: log file name for history output
        '''
        ...
        # Default parameters
        self.threshold = 0.005          # Tolerance on bound constraints satisfaction
    ...
    def iterate(self, x, fcost, grad, grad_preco = None):
        ...
        if fcost is np.nan:
            raise RuntimeError('cost function is nan')
    ...
    def project(self, x):
        """
        Smoothly clamps x to [lb, ub] with smoothstep tapering in [lb, lb+threshold] and [ub-threshold, ub].
        Zero values are ignored (treated as water S-wave speed).
        """
        
        lb, ub, threshold = self.lb, self.ub, self.threshold
        nonzero_mask = x != 0.0

        # Lower taper region: [lb, lb+threshold]
        threshold_lb = lb.min() * threshold
        mask_lower = (x >= lb) & (x < (lb + threshold_lb)) & nonzero_mask
        if np.any(mask_lower):
            t = (x[mask_lower] - lb[mask_lower]) / threshold_lb  # t in [0,1]
            taper = 3 * t**2 - 2 * t**3  # smoothstep        
            x[mask_lower] = lb[mask_lower] + taper * threshold_lb

        # Hard clip below lb
        mask_hard_lower = (x < lb) & nonzero_mask
        x[mask_hard_lower] = lb[mask_hard_lower]

        # Upper taper region: [ub-threshold, ub]
        threshold_up = ub.min() * threshold
        mask_upper = (x > (ub - threshold_up)) & (x <= ub) & nonzero_mask
        if np.any(mask_upper):
            t = (x[mask_upper] - (ub[mask_upper] - threshold_up)) / threshold_up  # t in [0,1]
            taper = 3 * t**2 - 2 * t**3  # smoothstep
            x[mask_upper] = (ub[mask_upper] - threshold_up) + taper * threshold_up

        # Hard clip above ub
        mask_hard_upper = (x > ub) & nonzero_mask
        x[mask_hard_upper] = ub[mask_hard_upper]
    ...
    def print_info(self, fcost):
        ...
        # ng = normL2(self.grad)
        # check the maximum gradient norm instead of the L2 norm
        ng = np.max(np.abs(self.grad))
        ...
        if(self.FLAG == 'INIT'):
            ...
            f.write('     Convergence criterion  : %10.4e\n' %self.conv)
            f.write('     Niter_max              : %10d\n'   %self.niter_max)
            f.write('     Initial cost is        : %10.4e\n' %self.f0)
            f.write('     Initial norm_grad is   : %10.4e\n' %ng)
            f.write('********************************************************************************\n')
            f.write('   Niter      fk         ||gk||       fk/f0        alpha        nls      ngrad    \n')
            f.write('%6d %12.4e %12.4e %12.2e %12.2e %8d %8d\n'
            %(self.cpt_iter+1, fcost, ng, fcost/self.f0, self.alpha, self.cpt_ls, self.nfwd_pb))
            f.close()

            print('     Convergence criterion  : %10.4e' %self.conv)
            print('     Niter_max              : %10d'   %self.niter_max)
            print('     Initial cost is        : %10.4e' %self.f0)
            print('     Initial norm_grad is   : %10.4e' %ng)
            print('********************************************************************************')
            print('   Niter      fk         ||gk||       fk/f0        alpha        nls      ngrad   \n')
            print('%6d %12.4e %12.4e %12.2e %12.2e %8d %8d'
            %(self.cpt_iter+1, fcost, ng, fcost/self.f0, self.alpha, self.cpt_ls, self.nfwd_pb))
            ...
            print('%6d %12.4e %12.4e %12.2e %12.2e %8d %8d'%(self.cpt_iter+1, fcost, ng, fcost/self.f0, 
    ...
    def print_info_TRN(self, fcost):
        ...
        if(self.FLAG=='INIT') :
            ...
            f.write('%6d %12.4e %12.4e %12.2e %12.2e %8d %8d %12.4e %8d %8d \n'%(
